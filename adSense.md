...
2.  **Populate Home Page:** Add content summarizing the website's purpose and origin story.
3.  **Add Call-to-Action:** Include a prominent button on the Home page that navigates the user to the main "Aartya" collection.
4.  **Consolidate Content:** Merge the "About Us" story into the Home page and remove the redundant "About" tab.
...
1.  ✅ **Group Short Items:** Render the "Shloka" tab as a single continuous reading page. Disable "Focus Mode" individual routing for Shlokas to avoid empty pages.
2.  ✅ **Sitemap Exclusion:** Exclude Shlokas from `sitemap.xml` so search engine bots don't crawl them as isolated pages.
...
- [x] *Step 1.5:* Create a rich, story-driven Home page and remove the redundant "About" tab. (Phase 0 Completed)
- [x] *Step 2:* Add the `description` rendering logic to the Aarti cards in `App.jsx` (Phase 1).
